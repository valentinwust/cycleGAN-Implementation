...
def get_opt():
    ...
    parser.add_argument("--log_batch_freq", type=int, default=10, help="how often (#batches) the log is printed to the console")
    parser.add_argument("--visual_batch_freq", type=int, default=1000, help="how often (#batches) current visuals should be saved")
    parser.add_argument("--save_epoch_freq", type=int, default=10, help="how often (#epochs) current model weights are saved")
    parser.add_argument('--use_eval', action='store_true', help='whether results for images from testA and testB should be periodically saved')
    parser.add_argument("--eval_epoch_freq", type=int, default=1, help="how often (#epochs) evaluation of test images should happen")
    # Optimizer
    ...
    parser.add_argument('--gpu_ids', type=int, nargs='+', default=[0], help='ids for GPUs, empty for CPU')
    parser.add_argument('--checkpoints_dir', type=str, default="checkpoints", help='path to checkpoint directory')
    parser.add_argument('--name', required=True, help='name of model, e.g. star_witcher')
    ...
    parser.add_argument('--load_model_name', type=str, default="_latest", help='name of loaded model(s)')

    parser.add_argument('--serial_batches', action='store_true', help='whether dataloader should not randomly select images from B')
    parser.add_argument('--no_flip', action='store_true', help='prevent data augmentation (flipping images), always True for test.py')
